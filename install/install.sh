...
install() {
  ...
  if [ "$MIXER_DISABLED" = "false" ]; then
	...
	kubectl apply -f https://raw.githubusercontent.com/iter8-tools/iter8-controller/v0.2.1/install/iter8-controller-telemetry-v2.yaml
  fi
  kubectl apply -f https://raw.githubusercontent.com/iter8-tools/iter8-analytics/v0.2.1/install/kubernetes/iter8-analytics.yaml
}
